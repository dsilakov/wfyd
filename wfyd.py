...
#            gobject.timeout_add(6000, self.task_cleanup_cbb)
    ...
    def on_projectbox_entry_changed(self, entrybox):
        ...
        if self.root.get(projectname):
            ...
            con = sqlite.connect(dbfile)
            cur = con.cursor()
            con.execute("UPDATE projects SET last_used=0")
            sql_stmt  = "UPDATE projects SET last_used=1 "
...
#          self.root.cur_project = projectname
    ...
    def on_deleteprojectbutton_clicked(self, button):
        """
        'Delete' button pressed - delete active project and all its tasks,
        ...
        if result in (gtk.RESPONSE_CLOSE, gtk.RESPONSE_REJECT):
            ...
            con = sqlite.connect(dbfile)
            cur = con.cursor()
            sql_stmt  = "DELETE FROM tasks WHERE project_id IN ("
    ...
    def on_quit1_activate(self, *args):
        """ Reaction to the 'Quit' menu item """
        self.root.save()
        gtk.main_quit()
...
#        label.set_text(status)
    ...
    def export_vcal(self, filename):
        """
        Save the whole history stored in db to a vcal file
        """
        vcal_file = open(filename, 'w')
        vcal_file.write("BEGIN:VCALENDAR\n")
        vcal_file.write("PRODID:-//plope.com/NONSGML WFYD//EN\n")
        vcal_file.write("VERSION:2.0\n")
        ...
        for projectname in self.root.get_projectnames():
            ...
            for entry in project.get_entries():
                begin = time.strftime('%Y%m%dT%H%M%S', time.localtime(entry.begin))
                end = time.strftime('%Y%m%dT%H%M%S', time.localtime(entry.end))
                notes = (entry.notes.replace('\\', '\\\\')
                         .replace(';', '\\;')
                         .replace(',', '\\,')
                         .replace('\r', '')
                         .replace('\n', '  '))
                vcal_file.write("BEGIN:VEVENT\n")
                vcal_file.write("UID:%s@%s\n" % (hash((begin, end, notes)), idhost))
                vcal_file.write("SUMMARY:[%s] %s\n" % (projectname, notes))
                vcal_file.write("DTSTART:%s\n" % begin)
                vcal_file.write("DTEND:%s\n" % end)
                vcal_file.write("DTSTAMP:%s\n" % dtstamp)
                vcal_file.write("END:VEVENT\n")
        vcal_file.write("END:VCALENDAR\n")
        vcal_file.close()

    def export_text(self, filename):
        """
        Save the whole history stored in db to a text file
        """
        ...
        for row in project:
            ...
            for row in task:
                ...
                end_time = datetime.datetime(end[0], end[1], end[2],
                                             end[3], end[4], end[5])

                begin_time = datetime.datetime(begin[0], begin[1], begin[2],
...
class EntryTree(object):
    ...
    def refresh(self, projectname):
        ...
        for entry in project.get_entries():
            ...
            self.store.set_value(item, 0, entry.begin)
            self.store.set_value(item, 1, time.strftime(ISO,
                                                        time.localtime(begin)))
            self.store.set_value(item, 2, duration)
            self.store.set_value(item, 3, minutes_repr(duration))
            self.store.set_value(item, 4, entry.notes)
    ...
    def on_entrytree_button_release_event(self, view, event):
        if event.button == 1:
            ...
            total_duration = 0
            for item in iters:
                duration = model.get_value(item, 2)
    ...
    def on_entry_delete_activate(self, menuitem):
        ...
        iters = [ model.get_iter(path) for path in paths ]
        for item in iters:
            self.delete_entry_row(model, item)

    def delete_entry_row(self, store, item):
        item_time = store.get_value(item, 0)
        ...
        con = sqlite.connect(dbfile)
        cur = con.cursor()
        sql_stmt =  "DELETE FROM tasks "
        sql_stmt += "WHERE time_start=datetime(" + str(item_time) + ", 'unixepoch')"
        cur.execute(sql_stmt)
        con.commit()
...
class JournalTree(object):
    ...
    def refresh(self, projectname):
        ...
        sql_stmt =  "SELECT task_name, strftime('%s', time_start), "
        sql_stmt += "       strftime('%s', time_finish) FROM tasks "
        sql_stmt += "WHERE "
        sql_stmt += "( (time_start <= datetime(" + str(date_finish) + ", 'unixepoch') "
        ...
        for row_tasks in cur_tasks:
            ...
            item = self.store.append()
            self.store.set_value(item, 0, int(row_tasks[1]))
            self.store.set_value(item, 1, time.strftime(ISO,
                                                        time.localtime(begin)))
            self.store.set_value(item, 2, duration)
            self.store.set_value(item, 3, minutes_repr(duration))
            self.store.set_value(item, 4, row_tasks[0])

    def on_journaltree_button_press_event(self, view, event):
        """
        Click on the 'Journal' button - if it is left-button click,
        then show Journals window, otherwise do nothing
        """
        if event.button != 3:
            # right button
            return
        ...
        if count < 1:
            ...
            popup = self.wtree.get_widget(
                                    'journal_entry_rightclick_popup_multi')
        popup.popup(None, None, None, event.button, event.time)
        return True
    ...
    def on_journal_entry_delete_activate(self, menuitem):
        ...
        iters = [ model.get_iter(path) for path in paths ]
        for item in iters:
            self.delete_journal_entry_row(model, item)

    def delete_journal_entry_row(self, store, item):
        time = store.get_value(item, 0)
        projectname = self.projectbox.get_child().get_text().strip()
        project = self.root.get(projectname)
        if not project:
            return
        ...
        for entry in project.get_entries():
            ...
            idx += 1
        sql_stmt =  "DELETE FROM tasks "
        sql_stmt += "WHERE time_start= datetime(" + str(time) + ", 'unixepoch')"
        store.remove(item)
...
class EntryEditWindow(object):
    ...
    def display(self):
        ...
        start = store.get_value(item, 0)
        minutes = store.get_value(item, 2) / 60
        notes = store.get_value(item, 4)
    ...
    def on_entry_edit_apply_clicked(self, *args):
        ...
        path = paths[0]
        item = store.get_iter(path)
        oldbegin = self.store.get_value(item, 0)
        projectname = self.projectbox.get_child().get_text().strip()
        project = self.root.get(projectname)
    ...
    def on_entry_edit_delete_event(self, *args):
        """
        'Edit Entry' popup - 'Cancel' is pressed;
...
class JournalEntryEditWindow(object):
    ...
    def on_journal_entry_edit_apply_clicked(self, *args):
        """
        'Edit Journal Entry' popup - 'Apply' is pressed;
        apply all the changes to the db and the entry box
        """
        begin = int(self.datebox.get_time())
        seconds = int(self.minutebox.get_value() * 60)
        buf = self.notesbox.get_buffer()
...
class PreferencesWindow(object):
    ...
    def on_prefs_ok_clicked(self, *args):
        """
        'Ok' is clicked in the Preferences window - apply the changes
        and hide the window (do not destroy it)
        """
    ...
    def on_prefs_cancel_clicked(self, *args):
        """
        'Cancel' is clicked in the Preferences window -
        do not destroy window, just hide it
        """
        self.hide()
...
class JournalsWindow(object):
    ...
    def on_Journals_delete_event(self, *args):
        """
        Journals window is going to be closed;
        we don't want it to be destroyed, just hide it
        """
        self.hide()
        return True
